console.log("Hello");
  ...
  async function FetchAllData( _callback){
    ...
    // for(var i=0;i<2;i++){
    await get(child(dbRef, "Maths/PPTs")).then((snapshot) => {
      if (snapshot.exists()) {
        list = (snapshot.val());
      } else {
  ...
  function insertLists(){
    if(list.count>0)
    {
        for(var i=1 ; i<=list.count ; i++){
          ...
          var element = document.getElementById("ppt");
          element.appendChild(content);
        }
        document.getElementById("pptsloading").style.display = "none";
        document.getElementById("pptslistHeader").style.display = "block";
      }
      else{
        document.getElementById("pptsloading").style.display = "none";
        document.getElementById("pptscomingsoon").style.display = "block";
      }
  }
